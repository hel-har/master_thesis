...
def group_supervised_contrastive_train_model(model, datasets, transformation_function, optimizer, index_mappings, distances, loss_function, strategy, args, weighted=False, epochs=100, verbose=0, patience=20, training=False, temporary_save_model_path=None):
    ...
    anchor_index_list, positive_indices, negative_indices = index_mappings
    anchor_index = 0
    epoch_wise_loss = []
    ...
    if args.adaptation_layer or args.adapt_mode_training:
        adapt_layers = []
    ...
    for epoch in range(epochs):
        ...
        if dds and args.multi_anchor:
            num_devices = len(anchor_index_list['train_devices'])
            if args.multi_anchor_training_mode == 'randomized': 
                anchor_index = np.random.choice(np.arange(num_devices))
            elif args.multi_anchor_training_mode == 'cycle':
                anchor_index = (epoch // args.multi_anchor_cycle) % num_devices
            elif args.multi_anchor_training_mode == 'divided':
                anchor_index = ((epoch // (epochs // num_devices)) % num_devices)
            print(f"Anchor device: {args.multi_anchor_list[anchor_index_list['train_devices'][anchor_index]]}")
        
        if args.adapt_mode_training:
            # change adapt_mode to false for all adaptation layers to enable it during training for specific layers dynamically
            for adapt_layer in adapt_layers:
                adapt_layer.adapt_mode = False

            num_adapt_layers = len(adapt_layers)
            print(f"Number of adaptation layers: {num_adapt_layers}")
            if args.multi_anchor_training_mode == 'randomized':
                adapt_layer_id = np.random.choice(np.arange(num_adapt_layers))
            elif args.multi_anchor_training_mode == 'cycle':
                adapt_layer_id = int(epoch // (args.multi_anchor_cycle+0.0001)) % num_adapt_layers
            elif args.multi_anchor_training_mode == 'divided':
                adapt_layer_id = ((epoch // (epochs // num_adapt_layers)) % num_adapt_layers)
            
            # enable adapt mode for selected adaptation layer
            print(f"Enabling adapt mode for adaptation layer: {adapt_layer_id}")
            adapt_layers[adapt_layer_id].adapt_mode = True 
        ...
        for enum, data_batch in enumerate(datasets):
            ...
            if dds:
                ...
                if args.multi_anchor_batches:
                    num_devices = len(anchor_index_list['train_devices'])
                    """if args.multi_anchor_training_mode == 'randomized': 
                        anchor_index = np.random.choice(anchor_index_list)
                    elif args.multi_anchor_training_mode == 'cycle':
                        anchor_index = anchor_index_list[(enum // args.multi_anchor_cycle) % num_devices]
                    elif args.multi_anchor_training_mode == 'divided':
                        num_batches = len(list(enumerate(datasets)))
                        anchor_index = anchor_index_list[(enum // (num_batches // num_devices) % num_devices)]"""
                    
                    #num_batches = len(list(enumerate(datasets)))
                    anchor_index = (enum // args.multi_anchor_cycle) % num_devices # always use 'cycle' training mode as it performed best in testing
                    

                positive_indices, negative_indices, distances = get_pos_neg(data_batch, anchor_index_list['train_devices'][anchor_index], strategy=strategy)
            ...
            with tf.GradientTape() as tape:
                ...
                for device_index, d in enumerate(data_batch):
                    if (device_index == anchor_index_list['train_devices'][anchor_index]) or (device_index in positive_indices) or (device_index in negative_indices):
                        if args.adaptation_layer:
                            embeddings.append(model({"input": np.array(d), "layer_selection": np.array([anchor_index_list['adapt_groups'][anchor_index]])}, training=training))
                        else:
                            embeddings.append(model(np.array(d), training=training))
                    else:
                        embeddings.append(None)
                anchor_embedding = embeddings[anchor_index_list['train_devices'][anchor_index]]
                ...
                if args.adaptation_layer:
                    a_id = anchor_index_list['adapt_groups'][anchor_index]
                    loss = loss_function(anchor_embedding, positive_embeddings, positive_weights, multi_negative_embeddings, negative_weights, data_batch, a_id, model)
                else:
                    loss = loss_function(anchor_embedding, positive_embeddings, positive_weights, multi_negative_embeddings, negative_weights)

            gradients = tape.gradient(loss, model.trainable_variables)

            """print("Adapt mode:", model.layers[4].adapt_mode)
            print("Anchor index (adapt groups):",anchor_index_list['adapt_groups'][anchor_index])
            for i, conv in enumerate(model.layers[4].sub_layers):
                for w in conv.trainable_variables:
                    ...
                    for j, var in enumerate(model.trainable_variables):
                        if var is w:
                            ...
                            break
                    grad = gradients[idx]
                    print(f"Conv branch {i}, weight {w.name}, grad norm = {tf.norm(grad).numpy()}")"""
